...
function makeWord(lettersObject) {
  const valuesArr = Object.values(lettersObject)
    .flat()
    .sort((a, b) => a - b);
  const objectArray = Object.entries(lettersObject);
  let str = '';
  valuesArr.forEach((number) => {
    objectArray.forEach((pair) => {
      if (pair[1].some((element) => element === number) === true) {
        str += pair[0];
      }
    });
  });
...
function sortCitiesArray(arr) {
  const countriesSortedArr = arr.sort((a, b) =>
    a.country > b.country ? 1 : -1
  );
  return countriesSortedArr.sort(function sor(a, b) {
